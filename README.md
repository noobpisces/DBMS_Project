...
2. [Relational Schemas](#2-relational-schemas)
3. [Các ràng buộc cần có](#3-các-ràng-buộc-cần-có)
4. [Cài đặt các CSDL và các ràng buộc](#4-cài-đặt-các-csdl-và-các-ràng-buộc)
   - [Tạo các bảng](#41-t%E1%BA%A1o-c%C3%A1c-b%E1%BA%A3ng)
   - [Tạo các view](#42-t%E1%BA%A1o-c%C3%A1c-view)
   - [Các trigger](#43-c%C3%A1c-trigger)
     	- [Trigger ngăn chặn học viên đăng kí lớp học trùng lịch, môn, quá 35 tín chỉ hoặc lớp đã đủ số lượng sinh viên (mặc định là 75)](#431-trigger-ngăn-chặn-học-viên-đăng-kí-lớp-học-trùng-lịch-môn-quá-35-tín-chỉ-hoặc-lớp-đã-đủ-số-lượng-sinh-viên-mặc-định-là-75)
     	- [Trigger ngăn trùng lịch học khi học viên chuyển lớp](#432-trigger-ngăn-trùng-lịch-học-khi-học-viên-chuyển-lớp)
    	- [Trigger hỗ trợ người quản lý xem xét mở lớp học mới](#433-trigger-hỗ-trợ-người-quản-lý-xem-xét-mở-lớp-học-mới)
     	- [Trigger hỗ trợ người quản lý xóa lớp không đủ điều kiện (có ít hơn 10 học viên đăng ký)](#434-trigger-hỗ-trợ-người-quản-lý-xóa-lớp-không-đủ-điều-kiện-có-ít-hơn-10-học-viên-đăng-ký)
     	- [Trigger hỗ trợ người quản lý xóa giảng viên](#435-trigger-hỗ-trợ-người-quản-lý-xóa-giảng-viên)
     	- [Trigger kiểm tra yêu cầu mở lớp của giảng viên](#436-trigger-kiểm-tra-yêu-cầu-mở-lớp-của-giảng-viên)
     	- [Trigger hỗ trợ admin tạo tài khoản](#437-trigger-hỗ-trợ-admin-tạo-tài-khoản)
     	- [Trigger hỗ trợ admin đổi mật khẩu](#438-trigger-hỗ-trợ-admin-đổi-mật-khẩu)
   - [Tạo user và phân quyền](#44-t%E1%BA%A0o-user-v%C3%A0-ph%C3%A2n-quy%E1%BB%81n)
5. [Các chức năng có trong project](#5-c%C3%A1c-ch%E1%BB%A9c-n%C4%83ng-c%C3%B3-trong-project)
   - [Chức năng của người quản lý](#51-ch%E1%BB%A9c-n%C4%83ng-c%E1%BB%A7a-ng%C6%B0%E1%BB%9Di-qu%E1%BA%A3n-l%C3%BD)
     	- [Kiểm tra tài khoản mật khẩu của người quản lý](#511-kiểm-tra-tài-khoản-mật-khẩu-của-người-quản-lý)
     	- [Tạo tài khoản](#512-tạo-tài-khoản)
      	- [Đổi mật khẩu](#513-đổi-mật-khẩu)
      - [Quản lý giảng viên](#514-quản-lý-giảng-viên)
      ...
      - [Quản lý học viên](#515-quản-lý-học-viên)
      ...
      - [Quản lý lớp học](#516-quản-lý-lớp-học)
   ...
   - [Chức năng của học viên](#52-chức-năng-của-học-viên)
      - [Đăng nhập vào hệ thống](#521-đăng-nhập-vào-hệ-thống)
      - [Xem các môn có thể đăng kí](#522-xem-các-môn-có-thể-đăng-kí)
      - [Xem các môn đã đăng kí](#523-xem-các-môn-đã-đăng-kí)
      - [Xem danh sách lớp của một môn](#524-xem-danh-sách-lớp-của-một-môn)
      - [Học viên đăng ký lớp học](#525-học-viên-đăng-ký-lớp-học)
      - [Học viên xóa đăng ký lớp học](#526-học-viên-xóa-đăng-ký-lớp-học)
      - [Học viên chuyển lớp](#527-học-viên-chuyển-lớp)
      - [Học viên xem danh sách học phí](#528-học-viên-xem-danh-sách-học-phí)
      - [Học viên xem tổng học phí](#529-học-viên-xem-tổng-học-phí)
      - [Tìm kiếm các lớp đang mở của một môn](#5210-tìm-kiếm-các-lớp-đang-mở-của-một-môn)
   - [Chức năng của giảng viên](#53-chức-năng-của-giảng-viên)
      - [Đăng nhập vào hệ thống](#531-đăng-nhập-vào-hệ-thống)
      - [Xem danh sách lớp dạy](#532-xem-danh-sách-lớp-dạy)
      - [Xem danh sách học viên của lớp đang dạy](#533-xem-danh-sách-học-viên-của-lớp-đang-dạy)
      - [Gửi đề xuất mở lớp](#534-gửi-để-xuất-mở-lớp)
...
```
### 4.2. Tạo các view
#### 4.2.1. Xem danh sách các lớp học phần sinh viên có thể đăng kí
```sql
...
```
#### 4.2.2. Xem danh sách các môn đã đăng kí
```sql
...
```
#### 4.2.3. Xem danh sách giảng viên nào dạy lớp nào
```sql
...
```
#### 4.2.4. Xem sinh viên nào đăng kí lớp nào
```sql
...
```
### 4.3. Các trigger 
#### 4.3.1. Trigger ngăn chặn học viên đăng kí lớp học trùng lịch, môn, quá 35 tín chỉ hoặc lớp đã đủ số lượng sinh viên (mặc định là 75)
```sql
...
```
#### 4.3.2. Trigger ngăn trùng lịch học khi học viên chuyển lớp
```sql
...
#### 4.3.3. Trigger hỗ trợ người quản lý xem xét mở lớp học mới
```sql
...
```
#### 4.3.4. Trigger hỗ trợ người quản lý xóa lớp không đủ điều kiện (có ít hơn 10 học viên đăng ký)
```sql
...
END;
```
#### 4.3.6. Trigger kiểm tra yêu cầu mở lớp của giảng viên
```sql
...
END;
```
#### 4.3.7. Trigger hỗ trợ admin tạo tài khoản
```sql
...
```
#### 4.3.8. Trigger hỗ trợ admin đổi mật khẩu
```sql
...
```
### 4.4. TẠO USER VÀ PHÂN QUYỀN
Ứng với ba nhóm người dùng (người quản lý, giảng viên, học viên) là role riêng
#### 4.4.1. Đối với role người quản lý (admin) ta sử dụng server role sysadmin:
